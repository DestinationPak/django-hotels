"""
P9.3 regression coverage: Location's swappable-model wiring and the
LocationAdapter contract.

The 0003 migration's own city -> location backfill matching logic was
covered here while Hotel.city still existed on the live model; that
field is gone now (see the 0004 migration), so those cases no longer
have anything live to run against.
"""

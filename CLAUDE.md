...
mechanism `django_trips.Location` uses. `Hotel.location` is now the only location field on
`Hotel` - the original free-text `Hotel.city` `CharField` has been dropped
(`migrations/0004_remove_hotel_city.py`), after `migrations/0003_backfill_hotel_locations.py`
best-effort backfilled `location` from `city` by name. `django_hotels/location_adapter.py`
...
`HotelListSerializer`/`HotelDetailSerializer` now expose `location` as a nested object through
the adapter, and `HotelFilter`'s public `?city=` query param filters on `location__name`
under the hood.

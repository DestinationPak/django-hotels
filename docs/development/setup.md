...
All development happens inside Docker; there is no supported bare-metal workflow. The importable app is `src/django_hotels/`; `devsite/` is a throwaway Django project shell (`urls.py`/`wsgi.py`/`asgi.py`) used only for local dev and not part of the published package.
...
```bash
pytest django_hotels/tests/test_services.py
pytest django_hotels/tests/test_services.py::SomeTestCase::test_name
```
...
- Write tests as `django.test.TestCase` subclasses, one class per model or concern, not bare `@pytest.mark.django_db` functions. `tests/test_models.py` predates this and hasn't been converted; don't copy its function style.
- Build fixtures with `django_hotels/tests/factories.py` (`HotelOwnerFactory`, `HotelFactory`, `HotelRoomTypeFactory`, `HotelAvailabilityFactory`, `HotelBookingFactory`, `UserFactory`), not `Model.objects.create(...)`. A raw `create()` is fine when the test is about model/manager mechanics (slug or booking-number generation, an `.active()` filter).

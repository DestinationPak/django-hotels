...
- Migrated packaging from `setup.py`/`setup.cfg`/`MANIFEST.in` to a single
  ...
  django_hotels/` (the throwaway dev project shell is now `devsite/`,
  renamed from `django-hotels/` to stop the two from being confused with
...
- `[tool.setuptools.packages.find]`'s `exclude` alone doesn't stop
  ...
  `django_hotels.api.tests` (this package's own internal API test suite,
  not documented as consumer-facing) was leaking into the built wheel
  despite being listed under `exclude`. `include-package-data` is now
  explicitly disabled; `django_hotels.tests` (the factories module, which
  *is* documented as consumer-facing) still ships as intended.
- Removed the `Makefile`'s `publish.test`/`publish.prod` targets - a
  ...
  src/django_hotels`) if run today.
...
### Added
- Availability search endpoint (`availabilities/`), filterable by
  `?hotel=`, `?room_type=`, `?date_from=`, `?date_to=` in any combination.
- Additional `HotelViewSet` catalog filtering (`?owner=`, `?location=`).
...
### Removed
- `Hotel.city` (the original free-text location `CharField`), now that
  `Hotel.location` is fully wired up. A prior migration best-effort
  backfilled `location` from `city` by name before this field was dropped.
...
### Added
- Authenticated booking retrieve/update endpoint.
- Model factories for tests (`django_hotels/tests/factories.py`), matching
  `django-trips`' testing convention - documented as the convention for
  all new tests going forward.

### Fixed
- Guest booking lookup no longer allows enumerating bookings by reference
  number alone (IDOR) - lookup now also requires the booking's own email
  (or an OTP).
- `HotelBookingCreateSerializer` now correctly sets `created_by` for
  authenticated users, not just guests.

...
Thanks for considering a contribution to `django-hotels`.
...
- Write tests as `django.test.TestCase` subclasses (see `CLAUDE.md`'s "Testing
  conventions" for the full rationale and the two legacy files that predate it).
- Build fixtures via `django_hotels/tests/factories.py`, not `Model.objects.create(...)`

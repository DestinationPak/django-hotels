...
.PHONY: requirements update_db random_hotels static help test build pull \
...
random_hotels: ## Adds random hotels
	$(MANAGE_PY_PATH) generate_hotels --batch_size=10
...
test: ## Run unit tests for the Hotels app
...
django-shell: ## Run django shell
	docker exec -it hotels.web /bin/bash

db-shell: ## Run mysql shell
	docker exec -it hotels.db /bin/bash
...
_clean_up:
	...
	rm -rf src/ dist/ build/ django_hotels.egg-info
